...
#Get-ChildItem -include *.flac,*.ogg,*.mp3,*.mod -name | Out-File -Encoding UTF8 $file_playlist_m3u


#Get-ChildItem -include *.flac,*.ogg,*.mp3,*.mod -name | % { $_.FullName } | Out-File -Encoding utf8 $file_playlist_m3u
#Get-ChildItem -include *.flac,*.ogg,*.mp3,*.mod -name | Out-File -Encoding utf8 $file_playlist_m3u | % { $_.FullName }
#Get-ChildItem -include *.flac,*.ogg,*.mp3,*.mod -name | Select-Object FullName | Out-File -Encoding UTF8 $file_playlist_m3u

Get-ChildItem *.* -include *.flac,*.ogg,*.mp3,*.mod | Select-Object FullName | Out-File -Encoding utf8 $file_playlist_m3u
# ^ funguje plna cesta 
sleep 1
