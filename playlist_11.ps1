...
$file_playlist_m3u = "playlistxxqq.txt"
...
#Get-ChildItem -Path $b2 *.* -Include $pole_include -Name| Out-File -Encoding UTF8 $file_playlist_m3u

#3echo $b2 

$qq = @(Get-ChildItem -Path $b2 *.* -Include $pole_include -Name)

#echo $qq

$file_obsah_m3u = @()

for ($aa=0;$aa -le $qq.Length-1; $aa++){
$vloz_2 = $b2
$vloz_2 += $qq[$aa]
echo $vloz_2
$file_obsah_m3u += $vloz_2
}
