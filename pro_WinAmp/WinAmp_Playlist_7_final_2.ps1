...
$pole_pripony = @("*.flac" ,"*.ogg" ,"*.mp3", "m4a") # v praxi zkouseno jenom z mp3 :) nic jinyho nebyl po ruce

$files = @() # pokud je pouze jeden soubor mp3 dela bordel, takze takto !!! ( type string, misto array )
$files += @(Get-ChildItem -Path $cesta_2 *.* -Include $pole_pripony -Name) | Sort-Object # += zajisti vzdu array variable type
#echo $files
